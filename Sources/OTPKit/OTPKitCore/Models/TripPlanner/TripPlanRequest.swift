...
extension TripPlanRequest {
    /// Creates a TripPlanRequest with specified parameters
    /// - Parameters:
    ///   - origin: The origin coordinate
    ///   - destination: The destination coordinate
    ///   - date: The date of travel (defaults to current date)
    ///   - time: The time of travel (defaults to current time)
    ///   - transportModes: The transport modes (defaults to transit and walk)
    ///   - maxWalkDistance: The maximum walk distance in meters (defaults to 1000)
    ...
    /// Creates a TripPlanRequest from Location objects
    /// - Parameters:
    ...
    ///   - arriveBy: Whether to arrive by the specified time (defaults to false)
    /// - Returns: A configured TripPlanRequest
    public static func fromLocations(
        origin: Location,
